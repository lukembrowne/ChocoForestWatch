...
class TestModelPredictorPredictAOI:
    """
    Test the predict_aoi method.
    
    This method gets COG URLs from extractor and predicts each one.
    We mock the extractor and the _predict_single_cog method.
    """
    ...
    def test_predict_aoi_with_multiple_cogs(self, mock_predictor, sample_aoi_geojson, temp_prediction_dir):
        """Test predict_aoi with multiple COGs found in AOI."""
        # Mock the _predict_single_cog method to return file paths
        with patch.object(mock_predictor, '_predict_single_cog') as mock_predict_single:
            ...
            # Act
            result = mock_predictor.predict_aoi(
            ...
            # Should call _predict_single_cog for each COG URL
            assert mock_predict_single.call_count == 2

    def test_predict_aoi_with_no_cogs_found(self, mock_predictor, sample_aoi_geojson, temp_prediction_dir):
        """Test predict_aoi when no COGs are found in AOI."""
        # Mock extractor to return empty list
        mock_predictor.extractor.get_cog_urls.return_value = []
    ...
    def test_predict_aoi_calls_extractor_correctly(self, mock_predictor, sample_aoi_geojson, temp_prediction_dir):
        """Test that predict_aoi calls extractor with correct parameters."""
        with patch.object(mock_predictor, '_predict_single_cog') as mock_predict_single:
            mock_predict_single.return_value = temp_prediction_dir / "test.tiff"
            
            mock_predictor.predict_aoi(
                aoi_geojson=sample_aoi_geojson,
                basemap_date="2022-01",
                pred_dir=temp_prediction_dir,
            ...
            # Should call extractor with correct collection and AOI
            mock_predictor.extractor.get_cog_urls.assert_called_once()
            call_args = mock_predictor.extractor.get_cog_urls.call_args
            assert call_args[0][0] == "custom-collection"  # collection
            # AOI should be converted to shapely geometry
