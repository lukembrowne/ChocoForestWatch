    ...
    <div class="basemap-date-slider">
      <div class="slider-container">
        <div class="year-markers">
          <div v-for="year in years" :key="year" class="year-marker" :style="{ left: `${getYearPosition(year)}%` }">
            {{ year }}
          </div>
        </div>
        <q-slider
          ...
          label
          label-always
          :label-value="formatDate(selectedDate)"
        ...
        <div class="month-markers">
          <div v-for="(date, index) in dates" :key="index" class="month-marker" :class="{ 'has-data': hasTrainingData(date) }">
            {{ formatMonth(date) }}
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    setup() {
      ...
      const hasTrainingData = (date) => {
        // This is a placeholder. You'll need to implement the actual logic to check if there's training data for this date.
        return Math.random() > 0.5; // For demonstration purposes
      };
